...
## Implementation steps
 ...
 - Implemented import-map merging and injected mappings for `node:test` -> `/oyinbo/test-runner.js`, `node:assert` -> `/oyinbo/assert.js`, and `node:oyinbo/worker` -> `/oyinbo/worker-bootstrap.js` in `js/server.js`.
 ...
 - Injected the test-runner script into the client injector (`clientScript`) so the test primitives are available in injected page contexts.
    Reason: make test primitives available in pages without manual import. Result: pages gain `test`, `describe`, `it`, and `oyinboRunTests` in scope.
...
~~None of the unit test functionality works.~~

~~There is some completely fishy business in terms of page and worker names and keys, meaning REPL files even sometimes get crossed. Need to debug.~~

**✅ RESOLVED - Conflicting test runner implementations:**
- ~~[js/test-runner.js](../js/test-runner.js) exports inline stringified code~~
- ~~[js/modules/test-runner.js](../js/modules/test-runner.js) exports ESM~~
- ~~[js/client.js](../js/client.js) injects the inline version, creating duplicates~~
- **Status**: [js/test-runner.js](../js/test-runner.js) kept for test harness compatibility, exports module content for Node.js tests. Client does NOT inject inline, uses import maps.

**✅ RESOLVED - Worker naming mismatch:**
- ~~Registry sanitizes names (lowercase, transform)~~
- ~~Worker creation and lookup use raw names~~
- ~~Keys don't match, causing file crossover~~
- **Status**: Worker creation now uses `sanitizeName()` to match registry expectations. Both main and timeout handler use sanitized names.

**✅ RESOLVED - Test runner API issues:**
- ~~`oyinboRunTests()` wrong syntax, need standard `node:test` runner API~~
- ~~`only` option registered but not enforced during execution~~
- ~~`assert.throws()` and `assert.rejects()` don't validate error type/pattern~~
- ~~`assert.deepEqual()` uses `JSON.stringify` (fragile, misses functions, circular refs)~~
- ~~No registry clearing between runs (tests accumulate)~~
- **Status**: All implemented correctly in [js/modules/test-runner.js](../js/modules/test-runner.js):
  - `only` enforcement: lines 104-115
  - `assert.throws()` validation: lines 249-267
  - `assert.rejects()` validation: lines 268-286
  - `assert.deepEqual()` uses proper algorithm: lines 289-335
  - Registry clearing: line 177

**✅ RESOLVED - Missing:**
- ~~Worker timeout diagnostics not written to chat logs~~
- ~~Test file import untested~~
- ~~Test registry realm isolation fails when `sessionStorage` unpopulated~~
- **Status**: 
  - Worker timeout diagnostics implemented in [js/server.js](../js/server.js) lines 232-241
  - Test file import supported via `files` array in `oyinboRunTests()`
  - Realm isolation uses fallback chain: sessionStorage → self.name → 'unknown'

**✅ RESOLVED - Remove:**
- ~~[js/test-runner.js](../js/test-runner.js)~~ - **Kept** for test harness, now properly documented
- ~~`testRunnerScript` from [js/client.js](../js/client.js) lines 3, 147-148~~ - **Removed** misleading comment
- ~~Import map entry `node:oyinbo/worker` (unused)~~ - **Removed** from server import map injection

**All issues resolved. System ready for integration testing.**
