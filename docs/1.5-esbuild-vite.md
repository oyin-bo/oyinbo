...
This document outlines a conceptual architecture for transforming Oyinbo's standalone HTTP server into plugins/extensions for esbuild and Vite. The goal is to enable the same file-based REPL capabilities within existing development server workflows, allowing agents to interact with pages served by these popular build tools.

**Key Insight**: The core REPL logic (registry, job queue, parser, writer, file watcher) requires **no changes**. The challenge is purely integration: injecting client scripts into HTML responses and exposing the `/oyinbo` HTTP endpoint within each tool's runtime environment.
...
2. **HTTP endpoints**: `/oyinbo` for polling (GET) and results (POST)
...
- Polls `/oyinbo` endpoint for code to execute
...
2. **Endpoint Exposure**: Handle GET/POST requests at `/oyinbo` path
...
- Acknowledge limitation: users not using serve API would need to manually include Oyinbo script in HTML or use external tool
...
Client script polls `/oyinbo?name=<page>&url=<url>` for code and POSTs results to same path. The endpoint must:
...
- Add middleware with `server.middlewares.use('/oyinbo', handler)`
...
configureServer(server) {
  ...
  server.middlewares.use('/oyinbo', (req, res) => {
...
- Check if path starts with `/oyinbo`, handle accordingly
...
- Client script polls `http://localhost:8301/oyinbo` instead of same-origin
- Requires CORS headers on responses

**Approach 3: Proxy Configuration**:
- User configures external proxy (nginx, etc.) to route `/oyinbo` to separate Oyinbo server
...
- Oyinbo watches `debug/*.md` files for agent requests
...
// Called when page first polls /oyinbo endpoint
...
- Oyinbo watcher detects change and creates job (correct)
...
- `configureServer`: Add `/oyinbo` middleware
...
- User must configure serve API themselves and include Oyinbo handler
...
**Philosophy**: Oyinbo features should degrade gracefully if environment doesn't support them.
...
- Prefix all logs with `[oyinbo]` for filterability
...
- Oyinbo plugin should probably run late (`enforce: 'post'`) to avoid conflicts
- But not so late that HTML is finalized and immutable

**Multiple Servers**:
- Vite can run multiple dev servers (SSR + client)
- Need to ensure Oyinbo doesn't double-register or conflict between servers
- May need to detect server type and skip SSR server

#### Recommended Plugin Structure (Conceptual)

```
export default function oyinboPlugin(options = {}) {
...
- User adds `onRequest` handler that checks path and delegates to Oyinbo
...
```
export function oyinboPlugin(options = {}) {
  const root = options.root || process.cwd();
  ...
  return {
    ...
    setup(build) {
      ...
      console.log('[oyinbo] Run separate server: node oyinbo-server.js');
      console.log('[oyinbo] Or configure esbuild.serve() with onRequest handler');
    }
  };
}
```
...
```
// oyinbo-server.js (provided by plugin package)
import { createServer } from 'http';
import * as registry from 'oyinbo/core/registry';
import * as job from 'oyinbo/core/job';
import * as watcher from 'oyinbo/core/watcher';
...
createServer((req, res) => {
  ...
  // ... handle /oyinbo GET/POST
}).listen(port);

console.log(`[oyinbo] Endpoint listening on http://localhost:${port}/oyinbo`);
```

User runs: `node oyinbo-server.js & esbuild --watch`
...
- Worker polls separate `/oyinbo` endpoint with unique name (`page-name-webworker`)
...
- Installation: `npm install oyinbo-vite-plugin` or `oyinbo-esbuild-plugin`
- Minimal config example
- How to verify it's working (check `debug.md`)
- Link to core Oyinbo docs for REPL usage
...
In complex setups (SSR, microservices), coordinate multiple Oyinbo instances.
...
**User Perspective**:
- Currently: Run `node js/cli.js` to start Oyinbo server
...
5. Verify `/oyinbo` endpoint responds
...
- Example: `oyinbo-core@1.0.0`, `oyinbo-vite-plugin@1.0.0` (depends on `oyinbo-core@^1.0.0`)
...
The transformation of Oyinbo from standalone server to plugin/extension for esbuild and Vite is **architecturally straightforward**: the core logic requires no changes. The challenge is purely integration, mapping each build tool's specific APIs onto the tool-agnostic core.
