...
- Expects `daebug/` directory and `daebug.md` file relative to ROOT
...
- Server starts, watches `daebug/`, serves files from project root
...
daebug/
  ...
  docs/                 (unchanged)
  daebug/                (example, not distributed)
```
...
[daebug] watching: daebug/*.md
  ...
  index.html connected for daebug/index-html.md
```
...
```
Excludes `daebug/`, `docs/`, test files.
...
**Options**:
- A: Auto-create `daebug/` directory if missing
- B: Require user to create `daebug/` manually
...
- Matches behavior of `registry.init()` which already creates `daebug.md`
...
5. User checks terminal, sees instructions to create `daebug/*.md` files
6. User creates `daebug/experiment.md`, writes code block
...
- `daebug.md` created in correct location
...
3. **Debug directory auto-creation**: Create empty `daebug/` or not?
