...
2. **Worker Registration**:
   ...
   - Important: the authoritative registry is the root `debug.md` file; the server treats only the per-instance files referenced in `debug.md` as active REPL logs. Simply creating files under `debug/` does not make them active unless they are listed in the master registry.
...
### Recommended Browser-Friendly Test Files for Dogfooding
...
4. **[`js/test-runner.test.js`](../../js/test-runner.test.js)** — Dogfooding the runner itself
   - Direct tests of `test()`, `describe()`, `it()`, assertion library, and `oyinboRunTests()`
