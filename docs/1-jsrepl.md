...
- Footer anchor: after all reply and background blocks, append a single-line footer starting with `> ` to provide an append anchor for editors and LLMs. For example:
  ...
  2. Agent appends request below footer: the server scans from the file bottom to find the canonical footer, reads the active tail region above it, and validates the presence of an agent request header and at least one complete fenced code block. If no complete fence is present the chunk is treated as a draft and is ignored.
